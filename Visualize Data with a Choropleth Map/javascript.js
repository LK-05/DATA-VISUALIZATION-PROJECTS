...
document.addEventListener("DOMContentLoaded", function() {
	...
	const buildChart = function() {
		...
		const legend = svg
			...
			.attr("transform", "translate(" + (w - paddingLeft - paddingRight) + ", " + (h - paddingBottom) + ")") // we move the whole group to where we want it, so that we don't have to also move the children elements one-by-one, thus saving on clutter and repeated code. Note that because we want the lowest value in our legend to be at the bottom, we have placed our legend in the bottom-right corner.
		;
